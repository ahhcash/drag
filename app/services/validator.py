...
class DocumentationValidator():
    ...
    async def _get_and_validate_response(self, client: httpx.AsyncClient, url: str) -> httpx.Response:
        response = await client.get(url, follow_redirects=True, timeout=10)
        response.raise_for_status()
        return response

    def _check_url_patterns(self, url: str, result: dict) -> dict:
    ...
    def _check_https_response(self, response: httpx.Response, result: dict) -> dict:
    ...
    def _analyze_content(self, soup: BeautifulSoup, result: dict) -> dict:
        ...
        meta_check = meta_description and any(s in meta_description.get('content', '').lower() for s in ["documentation", "technical guide", "api"]) # type: ignore
